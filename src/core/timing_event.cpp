...
static void DumpEvents()
{
  return;
  static u32 count = 0;
  printf("%u active events @ %u: \n", s_active_event_count, count);
  count++;

  for (TimingEvent* event = s_active_events_head; event; event = event->next)
  {
    printf("  %s: downcount %d, %d ticks since, interval %d, period %d\n", event->GetName().c_str(), event->m_downcount,
           event->m_time_since_last_run, event->m_interval, event->m_period);
  }
}
...
#if 0
      if (s_active_event_count > 1)
      {
        DebugAssert(event->next);
        event->next->prev = nullptr;
        s_active_events_head = event->next;
        event->prev = s_active_events_tail;
        event->next = nullptr;
        s_active_events_tail->next = event;
...
void TimingEvent::Schedule(TickCount ticks)
{
  ...
  else
  {
    ...
    if (!TimingEvents::s_running_events)
